...
#
"""
Removes unused (or unwanted) elements from a MuseScore3 score.
"""
...
def main():
	...
	if options.synth:
		for score in scores:
			if options.dry_run:
				print(f'Will delete per-channel synth settings for "{score.filename}"')
			else:
				score.remove_channel_synths()
				if options.verbose:
